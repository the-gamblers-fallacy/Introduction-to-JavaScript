...
function hungryDog(weight, age){
  if (age < 1) {
    if (age <= 4/12) {
      return weight * 0.1;
    } else if (age < 0.583) { // 7/12 gives a value of .583333, which makes the tests unhappy but ought to be correct...
      return weight * 0.05;
    } else {
      return weight * 0.04;
    }
...
function game(user = 0, computer){
  ...
  // put those into an arrary and convert answers into numbers based on their index
  ...
  if (user < computer) {
    ...
    // if the above case isn't true and there isn't a tie, user wins
    return "you win!";
  }
  if (user - computer === 1) {
    return "you win!";
  }
  // if every case up to this point doesn't run, computer wins
  return "you lose!";
}
...
function annoyingSong(numTimes){
...
function vowelCounter(userString) {
  ...
  return userString.toLowerCase().split("").filter(letter => {
